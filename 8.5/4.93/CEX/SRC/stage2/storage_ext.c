...
// For Trophy Unlocker
// It will patch current timestamp with random one reated in plugin while unlocking trophies
LV2_HOOKED_FUNCTION_COND_POSTCALL_4(int, sys_ss_secure_rtc, (uint32_t mode, uint64_t tick, uint64_t *hi_microseg, uint64_t *low_microseg))
{
	if(mode == 0x3002 && g_ticks_value)
	{			
		//DPRINTF("sys_ss_secure_rtc called. Patching with value 0x%lx\n", g_ticks_value);
		*hi_microseg = g_ticks_value;
		g_ticks_value = 0;
		return 0;
	}
...
void storage_ext_patches(void)
{
	...
	hook_function_with_cond_postcall(get_syscall_address(SYS_SS_SECURE_RTC), sys_ss_secure_rtc, 4);
...
void unhook_all_storage_ext(void)
{
	...
	unhook_function_with_cond_postcall(get_syscall_address(SYS_SS_SECURE_RTC), sys_ss_secure_rtc, 4);
