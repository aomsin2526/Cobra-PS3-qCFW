...
static uint8_t cfg_template[0x17] = 
{ 
...
static void check_and_correct(CobraConfig *cfg)
{
	...
	if(cfg->demo_c00_mode > 1)
		cfg->demo_c00_mode = 0;

	if(cfg->ps2_speed > 1 && cfg->ps2_speed < 0x45)
		cfg->ps2_speed = 0;
	...
	demo_c00_mode = config.demo_c00_mode;
}
...
int read_cobra_config(void)
{
	...
	/*DPRINTF("Configuration read.\n+ bd_video_region = %d\n+ dvd_video_region = %d\n+ fan_speed = %02X\n+ ps2_speed = %02X\n+ syscalls_mode = %02X\n+ gameboot_mode = %02X\n+ epilepsy_warning = %02X\n+ coldboot_mode = %02X\n+ hidden_trophy_mode = %02X\n+ demo_c00_mode = %02X\n\n",
		bd_video_region, dvd_video_region, fan_speed, ps2_speed, syscalls_mode, gameboot_mode, epilepsy_warning, coldboot_mode, hidden_trophy_mode, demo_c00_mode);*/
